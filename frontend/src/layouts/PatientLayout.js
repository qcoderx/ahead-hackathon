...
const PatientLayout = ({ children }) => {
  return (
    <div className="relative flex h-auto min-h-screen w-full flex-col group/design-root overflow-x-hidden patient-theme bg-background-light dark:bg-background-dark">
      <div className="layout-container flex h-full grow flex-col">
        {/* TopNavBar */}
        <header className="flex items-center justify-between whitespace-nowrap border-b border-solid border-border-light px-6 sm:px-10 py-3 bg-white dark:bg-background-dark dark:border-gray-700">
          <Link
          ...
          <div className="flex items-center gap-2">
            <span className="material-symbols-outlined text-text-subtle hidden sm:block">
              person
            </span>
            <span className="truncate text-sm font-bold text-text-main">
              Ayomide Williams (ID: P00451)
            </span>
            <Link to="/" className="ml-4 text-sm text-danger hover:underline">
              Logout
            </Link>
          </div>
        </header>
